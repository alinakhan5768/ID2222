...
public class CLI {
  ...
  public Config parseArgs(String[] args) throws FileNotFoundException {
    ...
    try {
      ...
      if (ANNEALING_SELECTION_POLICY.compareToIgnoreCase(AnnealingSelectionPolicy.LINEAR.toString()) == 0) {
        annealingSelectionPolicy = AnnealingSelectionPolicy.LINEAR;
      } else if (ANNEALING_SELECTION_POLICY.compareToIgnoreCase(AnnealingSelectionPolicy.EXPONENTIAL.toString()) == 0) {
        annealingSelectionPolicy = AnnealingSelectionPolicy.EXPONENTIAL;
      } else if (ANNEALING_SELECTION_POLICY.compareToIgnoreCase(AnnealingSelectionPolicy.IMPROVED_EXP.toString()) == 0) {
